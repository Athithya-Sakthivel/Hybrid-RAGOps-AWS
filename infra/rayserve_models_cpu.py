# CPU-first Ray Serve deployment for three models (embed, rerank, LLM).
# Hardcoded absolute paths:
#   Embed ONNX: /workspace/models/gte-modernbert-base/onnx/model_int8.onnx
#   Embed tokenizer: /workspace/models/gte-modernbert-base/tokenizer.json
#   Rerank ONNX: /workspace/models/ms-marco-TinyBERT-L2-v2/onnx/model_qint8_arm64.onnx
...
# Environment variables override computed defaults. LLM replicas default=1.

from __future__ import annotations
import os
cpu_count = os.cpu_count() or 1
_global_omp_threads = int(os.getenv("OMP_NUM_THREADS", str(max(1, cpu_count // 4))))
os.environ.setdefault("OMP_NUM_THREADS", str(_global_omp_threads))
os.environ.setdefault("OPENBLAS_NUM_THREADS", str(os.getenv("OPENBLAS_NUM_THREADS", str(_global_omp_threads))))
os.environ.setdefault("MKL_NUM_THREADS", str(os.getenv("MKL_NUM_THREADS", str(_global_omp_threads))))
os.environ.setdefault("VECLIB_MAXIMUM_THREADS", str(os.getenv("VECLIB_MAXIMUM_THREADS", str(_global_omp_threads))))

import sys, time, json, logging, traceback, queue, threading, concurrent.futures, asyncio
from typing import Dict, Any, Optional, List, Tuple
import numpy as np
import ray

# Ray Serve import + HTTPOptions compatibility handling
try:
    from ray import serve
except Exception:
    serve = None

# HTTPOptions: some versions expose it as ray.serve.HTTPOptions; older/newer minor variants exist.
try:
    from ray.serve import HTTPOptions  # preferred
except Exception:
    try:
        # fallback: attribute on serve module
        HTTPOptions = getattr(serve, "HTTPOptions")
    except Exception:
        # last-resort tiny shim that approximates the expected interface; serve.start will likely ignore unknowns but this prevents import error
        class HTTPOptions:
...
EMBED_NUM_CPUS_PER_REPLICA = _env_float("EMBED_NUM_CPUS_PER_REPLICA", _env_float("EMBED_NUM_CPUS_PER_REPLICA", 1.0))
...
LLM_MAX_REPLICAS_PER_NODE = _env_int("LLM_MAX_REPLICAS_PER_NODE", max(1, cpu_count))
...
class LlamaServe:
    def __init__(self, model_path: str = LLM_PATH, n_threads: int = LLM_N_THREADS, max_concurrency: int = LLM_MAX_CONCURRENCY):
        ...
        try:
            ...
            pass
        self._executor = concurrent.futures.ThreadPoolExecutor(max_workers=max(1, int(max(1, n_threads))))
...
def _build_deploy_options(prefix: str, fixed_replicas_env: int, default_num_gpus: float, default_num_cpus: float, max_replicas_per_node_env: int):
    ...
    if num_gpus != 0.0:
        # keep CPU-first deployments; force to 0.0 GPUs unless explicitly set to non-zero
        num_gpus = 0.0
    ...
    if mode == "auto":
        ...
        # Ray Serve expects `num_replicas="auto"` + autoscaling_config in recent versions
...
def deploy_binds(binds: List[Any]):
    """
    Attempt a few compatible ways to deploy a list of bound deployments to Serve.

    Recent Serve versions accept a single sequence to serve.run(list_of_bindings).
    Older versions may expect unpacked args. This helper tries a few variants while
    preserving the original behavior and raising meaningful errors on failure.
    """
    try:
        # preferred: serve.run accepts varargs in some versions
        serve.run(*binds)
        return
    except Exception:
        pass
    try:
        # preferred in many newer versions: serve.run accepts a single list
        serve.run(list(binds))
        return
    except Exception:
        pass
    # fall back to individually deploying binding objects
    for b in binds:
        try:
            if hasattr(b, "deploy"):
                b.deploy()
...
def main():
    ...
    # create HTTPOptions in a robust way (some Ray versions require different args)
    try:
        http_opts = HTTPOptions(host=SERVE_HTTP_HOST, port=SERVE_HTTP_PORT)
    except Exception:
        http_opts = HTTPOptions(SERVE_HTTP_HOST, SERVE_HTTP_PORT)

    # Start Serve (detached) — modern API uses serve.start with http_options
    try:
        serve.start(detached=True, http_options=http_opts)
    except TypeError:
        # fallback for versions that expect http_options named differently or not present
        try:
            serve.start(detached=True, http_options=http_opts)
        except Exception:
            # Best-effort: start without http_options
