...
if ONNX_USE_CUDA:
    ...
    if "CUDAExecutionProvider" not in providers_avail:
        raise RuntimeError("ONNX_USE_CUDA=true but CUDAExecutionProvider not available: " f"{providers_avail}. Make sure onnxruntime with CUDA support is installed and CUDA drivers are present.")
...
class ONNXEmbed:
    def __init__(self, onnx_path: str = ONNX_EMBED_PATH, tokenizer_path: str = ONNX_EMBED_TOKENIZER_PATH):
        ...
        log.info("[%s] startup OK: detected embed_dim=%d model_max_length=%s env_index_max=%d", EMBED_DEPLOYMENT, self._embed_dim, getattr(self.tokenizer, "model_max_length", None), INDEXING_EMBEDDER_MAX_TOKENS)
...
class ONNXRerank:
    def __init__(self, onnx_path: str = ONNX_RERANK_PATH, tokenizer_path: str = ONNX_RERANK_TOKENIZER_PATH):
        ...
        log.info("[%s] startup OK: sample scores shape %s model_max_length=%s env_cross_max=%d", RERANK_HANDLE_NAME, derived.shape, getattr(self.tokenizer, "model_max_length", None), CROSS_ENCODER_MAX_TOKENS)
    async def __call__(self, request_or_payload):
        try:
            body = None
            if isinstance(request_or_payload, (dict, list)):
