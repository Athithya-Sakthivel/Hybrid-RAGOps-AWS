...
LLM_PATH = os.getenv("LLM_PATH", "/workspace/models/Qwen3-0.6B-GGUF/Qwen3-0.6B-Q4_K_M.gguf")
    ...
    class ONNXEmbed:
        ...
        async def __call__(self, request_or_payload):
            try:
                ...
                try:
                    results = await __import__("asyncio").gather(*( __import__("asyncio").wrap_future(f) for f in futures_list))
                except Exception as e:
        ...
        def __del__(self):
            try:
                ...
                pass
    @serve.deployment(name=RERANK_HANDLE_NAME, num_replicas=RERANK_REPLICAS, ray_actor_options={"num_gpus": 0.0, "num_cpus": float(RERANK_NUM_CPUS_PER_REPLICA)})
    class ONNXRerank:
        def __init__(self, onnx_path: str = ONNX_RERANK_PATH, tokenizer_path: str = ONNX_RERANK_TOKENIZER_PATH):
            ...
            self.sess = make_session(onnx_path, intra_threads=intra, inter_threads=inter)
            self.input_names = [inp.name for inp in self.sess.get_inputs()]
            self._executor = __import__("concurrent.futures").ThreadPoolExecutor(max_workers=max(1, intra))
        ...
        def __del__(self):
            try:
                ...
                pass
    @serve.deployment(name=LLM_DEPLOYMENT_NAME, num_replicas=LLM_REPLICAS, ray_actor_options={"num_gpus": 0.0, "num_cpus": float(LLM_NUM_CPUS_PER_REPLICA)})
    class LlamaServe:
        def __init__(self, model_path: str = LLM_PATH, n_threads: int = LLM_N_THREADS, max_concurrency: int = LLM_MAX_CONCURRENCY):
            ...
            try:
                ...
                pass
            self._executor = __import__("concurrent.futures").ThreadPoolExecutor(max_workers=max(1, int(max(1, n_threads))))
...
def main():
    ...
    if os.getenv("GATEWAY_BIND_AUTO", "0") in ("1", "true", "True"):
        try:
            from infra.fastapi_gateway import GatewayIngress
            gateway_replicas = _env_int("GATEWAY_REPLICAS", 1)
            gateway_cpus = _env_float("GATEWAY_CPUS", 0.5)
            gateway_opts = {"num_replicas": gateway_replicas, "ray_actor_options": {"num_cpus": float(gateway_cpus), "num_gpus": 0.0}}
            gateway_bind = GatewayIngress.options(**gateway_opts).bind()
            binds.append(gateway_bind)
            log.info("Auto-bound GatewayIngress via fastapi_gateway.GatewayIngress")
        except Exception:
            log.exception("Gateway auto-bind failed")
    deploy_binds(binds)
    names = [EMBED_DEPLOYMENT, RERANK_HANDLE_NAME] + ([LLM_DEPLOYMENT_NAME] if _LLAMA_AVAILABLE else [])
