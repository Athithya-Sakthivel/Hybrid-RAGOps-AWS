...
RERANK_HANDLE_NAME = os.getenv("RERANK_HANDLE_NAME", "rerank_onnx_cpu")
...
def _ensure_file_bool(path: str) -> bool:
    if not path:
        return False
...
class ONNXRerank:
    def __init__(self, onnx_path: str = ONNX_RERANK_PATH, tokenizer_path: str = ONNX_RERANK_TOKENIZER_PATH):
        ...
        self.sess = make_session(onnx_path, intra_threads=intra, inter_threads=inter)
        self.input_names = [inp.name for inp in self.sess.get_inputs()]
        self._executor = cf.ThreadPoolExecutor(max_workers=max(1, intra))
        self._sem = asyncio.Semaphore(max(1, intra))
        self.ready = False
        try:
...
class LlamaServe:
    def __init__(self, model_path: str = LLM_PATH, n_threads: int = LLM_N_THREADS, max_concurrency: int = LLM_MAX_CONCURRENCY):
        if not _LLAMA_AVAILABLE:
            raise RuntimeError("llama-cpp-python not available; install llama-cpp-python to use LLM deployment")
        ...
        try:
            ...
            pass
        self._executor = cf.ThreadPoolExecutor(max_workers=max(1, int(max(1, n_threads))))
        ...
        try:
            ...
            raise
    async def generate(self, prompt: str, params: Optional[Dict[str, Any]] = None):
        params = params or {}
        async with self._sem:
            loop = asyncio.get_running_loop()
            try:
                result = await loop.run_in_executor(self._executor, lambda: self.llm(prompt, **params))
    ...
    async def stream(self, prompt: str, params: Optional[Dict[str, Any]] = None):
        ...
        async with self._sem:
            ...
            loop = asyncio.get_running_loop()
            while True:
                tok = await loop.run_in_executor(None, q_tokens.get)
                if tok is None:
                    break
                yield tok
    async def __call__(self, request):
        ...
        if not stream_flag:
            res = await self.generate(prompt, params=params)
            return res
        else:
...
def deploy_binds(binds: List[Any]):
    ...
    for b in binds:
        ...
        try:
            ...
            log.exception("Failed to deploy binding %s: %s", type(b), e); raise
def main():
    if ray is None or serve is None:
        raise RuntimeError("ray or ray.serve not importable")
    ...
    if ort is not None and PreTrainedTokenizerFast is not None and _ensure_file_bool(ONNX_EMBED_PATH) and _ensure_file_bool(ONNX_EMBED_TOKENIZER_PATH):
        try:
            EmbedDep = serve.deployment(name=EMBED_DEPLOYMENT, num_replicas=embed_opts.get("num_replicas", 1), ray_actor_options=embed_opts.get("ray_actor_options", {}))(ONNXEmbed)
            embed_bind = EmbedDep.options(**({"autoscaling_config": embed_opts.get("autoscaling_config")} if embed_opts.get("autoscaling_config") else {})).bind(ONNX_EMBED_PATH, ONNX_EMBED_TOKENIZER_PATH)
            binds.append(embed_bind)
        except Exception as e:
            skipped["embed"] = f"embed bind failed: {e}"
    else:
        skipped["embed"] = "onnxruntime or tokenizer or files missing"
    rerank_opts = _build_deploy_options("RERANK", RERANK_REPLICAS, 0.0, RERANK_NUM_CPUS_PER_REPLICA, 1)
    if ort is not None and PreTrainedTokenizerFast is not None and _ensure_file_bool(ONNX_RERANK_PATH) and _ensure_file_bool(ONNX_RERANK_TOKENIZER_PATH):
        try:
            RerankDep = serve.deployment(name=RERANK_HANDLE_NAME, num_replicas=rerank_opts.get("num_replicas", 1), ray_actor_options=rerank_opts.get("ray_actor_options", {}))(ONNXRerank)
            rerank_bind = RerankDep.options(**({"autoscaling_config": rerank_opts.get("autoscaling_config")} if rerank_opts.get("autoscaling_config") else {})).bind(ONNX_RERANK_PATH, ONNX_RERANK_TOKENIZER_PATH)
            binds.append(rerank_bind)
        except Exception as e:
            skipped["rerank"] = f"rerank bind failed: {e}"
    else:
        skipped["rerank"] = "onnxruntime or tokenizer or files missing"
    if _LLAMA_AVAILABLE and _ensure_file_bool(LLM_PATH):
        try:
            llm_opts = _build_deploy_options("LLM", LLM_REPLICAS, 0.0, LLM_NUM_CPUS_PER_REPLICA, max(1, cpu_count))
            LLMDep = serve.deployment(name=LLM_DEPLOYMENT_NAME, num_replicas=llm_opts.get("num_replicas", 1), ray_actor_options=llm_opts.get("ray_actor_options", {}))(LlamaServe)
            llm_bind = LLMDep.options(**({"autoscaling_config": llm_opts.get("autoscaling_config")} if llm_opts.get("autoscaling_config") else {})).bind(LLM_PATH, LLM_N_THREADS, LLM_MAX_CONCURRENCY)
            binds.append(llm_bind)
        except Exception as e:
            skipped["llm"] = f"llm bind failed: {e}"
    else:
        skipped["llm"] = "llama-cpp not available or model file missing"
    gateway_auto = os.getenv("GATEWAY_BIND_AUTO", "1") in ("1", "true", "True")
    if gateway_auto:
        try:
            import infra.fastapi_gateway as fg
            GatewayDep = serve.deployment(name=os.getenv("GATEWAY_DEPLOYMENT_NAME", "gateway"), num_replicas=_env_int("GATEWAY_REPLICAS", 1), ray_actor_options={"num_gpus": 0.0, "num_cpus": float(_env_float("GATEWAY_CPUS", 0.5))})(fg.GatewayIngress)
            gateway_bind = GatewayDep.bind()
            binds.append(gateway_bind)
        except Exception as e:
            skipped["gateway"] = f"gateway bind failed: {e}"
    try:
        if binds:
            deploy_binds(binds)
        else:
            log.warning("No binds created; nothing to deploy")
    except Exception as e:
        log.exception("deploy_binds failed: %s", e)
    names = []
    if (ort is not None and PreTrainedTokenizerFast is not None and _ensure_file_bool(ONNX_EMBED_PATH)): names.append(EMBED_DEPLOYMENT)
    if (ort is not None and PreTrainedTokenizerFast is not None and _ensure_file_bool(ONNX_RERANK_PATH)): names.append(RERANK_HANDLE_NAME)
    if (_LLAMA_AVAILABLE and _ensure_file_bool(LLM_PATH)): names.append(LLM_DEPLOYMENT_NAME)
    log.info("Deployments started: %s", " ".join(names))
    log.info("Serve HTTP listening on %s:%d", SERVE_HTTP_HOST, SERVE_HTTP_PORT)
    log.info("Skipped deployments: %s", json.dumps(skipped))
