...
try:
    ...
    raise ImportError("ray and ray.serve are required. Install ray; error: " + str(e))
try:
...
QDRANT_URL = os.getenv("QDRANT_URL", os.getenv("QDRANT_HTTP_URL", "http://127.0.0.1:6333"))
...
_executor = concurrent.futures.ThreadPoolExecutor(max_workers=int(os.getenv("QUERY_THREADPOOL_WORKERS", "8")))
...
def _ensure_ray_connected():
    if ray is None:
        raise RuntimeError("ray not available at runtime")
    if not ray.is_initialized():
        if RAY_ADDRESS and RAY_ADDRESS != "auto":
            ray.init(address=RAY_ADDRESS, namespace=RAY_NAMESPACE, ignore_reinit_error=False)
        else:
            ray.init(namespace=RAY_NAMESPACE, ignore_reinit_error=False)
def _resolve_ray_response(obj, timeout: float):
    try:
        return ray.get(obj, timeout=timeout)
    except Exception:
        return obj
def _call_handle_sync(handle, payload, timeout: float):
    try:
        if hasattr(handle, "remote"):
            return handle.remote(payload)
        return handle(payload)
    except Exception as e:
        raise RuntimeError("serve invocation failed: " + str(e))
async def _call_handle(handle, payload, timeout: float):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_executor, lambda: _call_handle_sync(handle, payload, timeout))
def call_handle(handle, payload, timeout: float = EMBED_TIMEOUT):
    if handle is None:
        raise RuntimeError("handle is None")
    resp = _call_handle_sync(handle, payload, timeout)
    try:
        ...
        try:
            ...
            if handle is None and serve is not None and hasattr(serve, "get_deployment"):
                try:
                    ...
                    handle = None
            if handle is None:
                last_exc = RuntimeError("no serve handle API available")
                time.sleep(poll)
                continue
            probe_payload = {"texts": ["__health_check__"], "max_length": int(min(8, INFERENCE_EMBEDDER_MAX_TOKENS))}
            try:
                _ = call_handle(handle, probe_payload, timeout=min(10.0, timeout))
                return handle
            except Exception as e:
                last_exc = e
        except Exception as e:
...
def make_clients() -> Tuple[Any, Any]:
    ...
    if QdrantClient is None:
        log.warning("qdrant-client not installed: %s", getattr(globals(), "_QDRANT_IMPORT_ERROR", ""))
    else:
        try:
    ...
    if GraphDatabase is None:
        log.warning("neo4j driver not installed: %s", getattr(globals(), "_NEO4J_IMPORT_ERROR", ""))
    else:
        try:
            ...
            log.warning("neo4j client creation failed: %s", e)
            neo = None
    if q is None and neo is None:
        raise RuntimeError("no datastore clients available: set QDRANT_URL and/or NEO4J_URI and ensure clients are installed")
    return q, neo
...
def retrieve_pipeline(embed_handle, rerank_handle, qdrant_client, neo4j_driver, query_text: str, max_chunks: int = MAX_CHUNKS_TO_LLM):
    ...
    try:
        ...
        vec = None
    ann_hits = []
    if vec is not None and qdrant_client is not None:
        try:
    ...
    texts_map = {}
    try:
        if MODE == "vector_only" and qdrant_client is not None:
            for h in ann_hits:
                payload = h.get("payload") or {}
                cid = payload.get("chunk_id") or payload.get("chunkId")
                if cid:
...
async def call_llm_stream(llm_handle, prompt_json: str, params: Optional[Dict[str, Any]] = None):
    ...
    try:
        ...
        if resp is None:
            try:
                txt = await asyncio.get_running_loop().run_in_executor(_executor, lambda: call_llm_blocking(llm_handle, prompt_json, params=params, timeout=CALL_TIMEOUT_SECONDS))
                yield str(txt)
                return
            except Exception as e:
        ...
        try:
            if hasattr(resp, "result") and callable(resp.result):
                iterable = await asyncio.get_running_loop().run_in_executor(_executor, lambda: resp.result(timeout=CALL_TIMEOUT_SECONDS))
            elif hasattr(resp, "get") and callable(resp.get):
                iterable = await asyncio.get_running_loop().run_in_executor(_executor, lambda: resp.get(timeout=CALL_TIMEOUT_SECONDS))
            else:
                try:
                    iterable = await asyncio.get_running_loop().run_in_executor(_executor, lambda: ray.get(resp, timeout=CALL_TIMEOUT_SECONDS))
                except Exception:
                    iterable = resp
        except Exception:
            iterable = resp
