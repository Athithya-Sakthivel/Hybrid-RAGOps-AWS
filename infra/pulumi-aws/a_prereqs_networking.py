...
try:
    ...
    kms_key = aws.kms.Key("raySsmKey", description=f"CMK for Ray SSM ({STACK})", deletion_window_in_days=30, tags={"Name": f"ray-ssm-key-{STACK}"})
...
if not NO_NAT:
    ...
    for idx, ps in enumerate(private_subnets):
        ...
        private_route_tables.append(prt)
pulumi_state_bucket = aws.s3.Bucket(f"pulumi-state-bucket", bucket=f"pulumi-state-{STACK}-{aws.get_caller_identity().account_id}", server_side_encryption_configuration=aws.s3.BucketServerSideEncryptionConfigurationArgs(rule=aws.s3.BucketServerSideEncryptionConfigurationRuleArgs(apply_server_side_encryption_by_default=aws.s3.BucketServerSideEncryptionConfigurationRuleApplyServerSideEncryptionByDefaultArgs(sse_algorithm="AES256"))), opts=pulumi.ResourceOptions(parent=vpc))
autoscaler_bucket = aws.s3.Bucket("autoscaler-bucket", bucket=(os.getenv("AUTOSCALER_BUCKET_NAME") or cfg.get("autoscalerBucketName") or f"ray-autoscaler-{STACK}-{aws.get_region().name}"), opts=pulumi.ResourceOptions(parent=vpc))
models_bucket = aws.s3.Bucket("models-bucket", bucket=(os.getenv("MODELS_S3_BUCKET") or cfg.get("modelsS3Bucket") or f"ray-models-{STACK}-{aws.get_region().name}"), opts=pulumi.ResourceOptions(parent=vpc))
...
alb_sg = aws.ec2.SecurityGroup(f"ray-alb-sg-{STACK}", vpc_id=vpc.id, description="ALB SG", ingress=[aws.ec2.SecurityGroupIngressArgs(protocol="tcp", from_port=443, to_port=443, cidr_blocks=["0.0.0.0/0"]), aws.ec2.SecurityGroupIngressArgs(protocol="tcp", from_port=80, to_port=80, cidr_blocks=["0.0.0.0/0"])], egress=[aws.ec2.SecurityGroupEgressArgs(protocol="-1", from_port=0, to_port=0, cidr_blocks=["0.0.0.0/0"])], tags={"Name": f"ray-alb-sg-{STACK}"})
head_sg = aws.ec2.SecurityGroup(f"ray-head-sg-{STACK}", vpc_id=vpc.id, description="Head SG", ingress=[aws.ec2.SecurityGroupIngressArgs(protocol="tcp", from_port=6379, to_port=6379, cidr_blocks=[VPC_CIDR]), aws.ec2.SecurityGroupIngressArgs(protocol="tcp", from_port=8003, to_port=8003, security_groups=[alb_sg.id]), aws.ec2.SecurityGroupIngressArgs(protocol="tcp", from_port=8003, to_port=8003, cidr_blocks=[VPC_CIDR])], egress=[aws.ec2.SecurityGroupEgressArgs(protocol="-1", from_port=0, to_port=0, cidr_blocks=["0.0.0.0/0"])], tags={"Name": f"ray-head-sg-{STACK}"})
worker_sg = aws.ec2.SecurityGroup(f"ray-worker-sg-{STACK}", vpc_id=vpc.id, description="Worker SG", ingress=[aws.ec2.SecurityGroupIngressArgs(protocol="tcp", from_port=8003, to_port=8003, security_groups=[alb_sg.id])], egress=[aws.ec2.SecurityGroupEgressArgs(protocol="-1", from_port=0, to_port=0, cidr_blocks=["0.0.0.0/0"])], tags={"Name": f"ray-worker-sg-{STACK}"})
