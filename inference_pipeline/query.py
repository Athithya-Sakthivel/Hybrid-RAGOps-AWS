...
# Qdrant / Neo4j / retrieval / ranking defaults
...
# === NEW LLM configuration (added) ===
LLM_DEPLOYMENT = os.getenv("LLM_DEPLOYMENT_NAME", "llm_server_cpu")
LLM_TIMEOUT = float(os.getenv("LLM_TIMEOUT", str(max(30.0, CALL_TIMEOUT_SECONDS))))
...
@retry()
def qdrant_vector_search(client: QdrantClient, q_vec: List[float], top_k: int, with_payload: bool = True, with_vectors: bool = True) -> List[Dict[str, Any]]:
    kw = {}
    kw["with_payload"] = bool(with_payload)
    kw["with_vector"] = bool(with_vectors)
    results = None
    last_exc = None
...
def neo4j_find_chunk_ids_by_qdrant_ids(driver, qdrant_ids: List[str]) -> Dict[str, str]:
    out = {}
...
# === Modified: retrieve_pipeline now optionally calls LLM and returns its result ===
def retrieve_pipeline(embed_handle, rerank_handle, qdrant_client, neo4j_driver, query_text: str, max_chunks: int = MAX_CHUNKS_TO_LLM, llm_handle=None, llm_params: Optional[Dict[str, Any]] = None, llm_timeout: Optional[float] = None) -> Dict[str, Any]:
    t0 = time.time()
    vec = None
    try:
    ...
    if ENABLE_METADATA_CHUNKS:
        try:
            if MODE == "vector_only":
                try:
                    meta_hits = qdrant_client.search(collection_name=QDRANT_COLLECTION, query_vector=None, with_payload=True, limit=MAX_METADATA_CHUNKS)
                    for mh in meta_hits:
    ...
    for cid in combined_unique_candidates:
        ...
        if cid in vec_map and cid in texts_map:
            rec["vector"] = None
        if cid in vec_map:
            pass
    ...
    # === NEW: call LLM if handle provided ===
    llm_result = None
    if llm_handle is not None:
        try:
            if llm_params is None:
                llm_params = {}
            # ensure sensible max_tokens default if not specified
            if "max_tokens" not in llm_params and LLM_MAX_TOKENS:
                llm_params["max_tokens"] = LLM_MAX_TOKENS
            # use call_handle which understands Ray Serve return types
            payload = {"prompt": prompt, "params": llm_params}
            to = float(llm_timeout) if llm_timeout is not None else float(LLM_TIMEOUT)
            res = call_handle(llm_handle, payload, timeout=to)
            # canonicalize res a bit
            if isinstance(res, dict):
                llm_result = res
            else:
                # try to coerce to dict with text under 'text' key
                try:
                    llm_result = {"text": str(res)}
                except Exception:
                    llm_result = {"text": None, "raw": repr(res)}
        except Exception as e:
            log.exception("LLM invocation failed: %s", e)
            llm_result = {"ok": False, "error": str(e)}
    # === END LLM call ===

    return {"prompt": prompt, "provenance": provenance, "records": llm_records, "llm": llm_result, "elapsed": elapsed}
...
def main():
    ...
    # === NEW: try to resolve LLM handle (optional) ===
    try:
        llm_handle = get_strict_handle(LLM_DEPLOYMENT, timeout=6.0, app_name=SERVE_APP_NAME)
        log.info("LLM handle resolved: %s", LLM_DEPLOYMENT)
    except Exception:
        log.info("LLM handle not resolved (LLM may be absent). Continuing without LLM")
        llm_handle = None
    # === END LLM resolve ===
    ...
    q = "health-check"
    try:
        # pass llm_handle and let retrieve_pipeline call it if available
        res = retrieve_pipeline(embed_handle, rerank_handle, qdrant_client, neo4j_driver, q, max_chunks=TOP_K, llm_handle=llm_handle, llm_params=None, llm_timeout=LLM_TIMEOUT)
